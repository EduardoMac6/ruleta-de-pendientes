(function () {
  ...
  const cfg = window.__RULETA_CONFIG__ || {};
  const supabaseUrl = (cfg.supabaseUrl || "").trim();
  const supabaseAnonKey = (cfg.supabaseAnonKey || "").trim();
  const hasSupabaseConfig = Boolean(supabaseUrl && supabaseAnonKey);
  const supabaseGlobal = typeof globalThis !== "undefined" ? globalThis.supabase : null;

  /** @type {any} */
  let supabaseClient = null;
  if (hasSupabaseConfig && supabaseGlobal && typeof supabaseGlobal.createClient === "function") {
  ...
  async function logActivityEvent(eventType, payload) {
    ...
    if (error) {
      console.warn("[ruleta] activity_events insert", error.message);
    }
  ...
  async function loadHistoryAndScore() {
    ...
    const [histRes, scoreRes] = await Promise.all([
      supabaseClient
        ...
        .limit(50),
      supabaseClient.rpc("user_activity_score"),
    ]);

    const { data, error } = histRes;
    if (error) {
      console.warn("[ruleta] load events", error.message);
      return;
    }

    const rows = data || [];
    let totalScore = 0;
    const rawScore = scoreRes.data;
    if (!scoreRes.error && rawScore != null && !Number.isNaN(Number(rawScore))) {
      totalScore = Number(rawScore);
    } else {
      if (scoreRes.error) console.warn("[ruleta] user_activity_score", scoreRes.error.message);
      totalScore = scoreFromEvents(rows);
    }
  ...
  formSignup.addEventListener("submit", async (e) => {
    ...
    if (data.session) {
      ...
      setAuthInfo("Revisa tu correo para confirmar la cuenta, si tu proyecto lo requiere.");
    }
  });
  ...
  [authEmailIn, authPassIn, authEmailUp, authPassUp].forEach((el) => {
    el.addEventListener("input", () => setAuthError(""));
  });

  /** Roulette + list (runs once) */
